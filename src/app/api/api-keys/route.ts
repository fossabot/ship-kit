  ...
  try {
    ...
    const userApiKeys = await db
      ...
      .where(
        inArray(
          apiKeys.projectId,
          userProjects.map((up) => up.project.id),
        ),
      );
  ...
  try {
    ...
    if (isTestKey) {
      ...
      if (userProjects.length === 0) {
        ...
        try {
          console.log(
            `Attempting to add user to test project: ${testProject.id}`,
          );
          const newProjectMember = await db
            ...
            .returning();
          console.log(
            `User added as member of test project: ${testProject.id}`,
            newProjectMember,
          );
        } catch (error) {
          ...
          return NextResponse.json(
            { error: "Failed to add user to test project" },
            { status: 500 },
          );
        }
      } else if (!projectId) {
        ...
        const hasAccess = userProjects.some(
          (up) => up.project.id === projectId,
        );
        if (!hasAccess) {
          console.log(`User does not have access to project: ${projectId}`);
          return NextResponse.json(
            { error: "User does not have access to the specified project" },
            { status: 403 },
          );
        }
      }
